...
from libs.roialign.roi_align.crop_and_resize import CropAndResizeFunction
...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, gt_bin_drawings, config):
    ...
    gt_class_ids: [batch, N_GT_INSTANCES] Integer class IDs.
    gt_boxes: [batch, N_GT_INSTANCES, (y1, x1, y2, x2)] in normalized
              coordinates.
    gt_masks: [batch, N_GT_INSTANCES, height, width] of boolean type
    gt_bin_drawings: [batch, N_GT_INSTANCES, height, width] of boolean type
    ...
    gt_bin_drawings = gt_bin_drawings.squeeze(0)
    ...
    if torch.nonzero(gt_class_ids < 0).size()[0]:
        ...
        gt_bin_drawings = gt_bin_drawings[non_crowd_ix.data, :]
    ...
    if torch.nonzero(positive_roi_bool).size()[0]:
        ...
        roi_masks = gt_masks[roi_gt_box_assignment.data, :, :]  # [nPosROIs, height, width]
        roi_bin_drawings = gt_bin_drawings[roi_gt_box_assignment.data, :, :]  # [nPosROIs, height, width]
        ...
        bin_drawings = Variable(
            CropAndResizeFunction(config.MASK_SHAPE[0], config.MASK_SHAPE[1], 0)(roi_bin_drawings.unsqueeze(1), boxes,
                                                                                 box_ids).data, requires_grad=False)
        masks = masks.squeeze(1)
        bin_drawings = bin_drawings.squeeze(1)

        # Threshold mask pixels at 0.5 to have GT masks be 0 or 1 to use with
        # binary cross entropy loss.
        masks = torch.round(masks)
        bin_drawings = torch.round(bin_drawings)
    else:
        positive_count = 0
    ...
    if positive_count > 0 and negative_count > 0:
        ...
        bin_drawings = torch.cat([bin_drawings, zeros], dim=0)
        ...
        bin_drawings = Variable(torch.FloatTensor(), requires_grad=False)
        if config.GPU_COUNT:
            rois = rois.cuda()
            roi_gt_class_ids = roi_gt_class_ids.cuda()
            deltas = deltas.cuda()
            masks = masks.cuda()
            bin_drawings = bin_drawings.cuda()
...
def compute_mrcnn_mask_loss(target_masks, target_class_ids, pred_masks, target_bin_drawings, ignore_bg=False):
    """Mask binary cross-entropy loss for the masks head.

    target_masks: [num_rois, height, width], e.g. [200, 28, 28].
    ...
    pred_masks: [proposals, num_classes, height, width] float32 tensor
                with values from 0 to 1, e.g. [200, 47, 28, 28].
    target_bin_drawings: [num_rois, height, width], e.g. [200, 28, 28].
    """
    if target_class_ids.size()[0]:
        # Only positive ROIs contribute to the loss. And only
        ...
        y_true = target_masks[indices[:, 0].data, :, :]  # [nPosRoIs, h, w]
        y_bin = target_bin_drawings[indices[:, 0].data, :, :]  # [nPosRoIs, h, w]
        y_pred = pred_masks[indices[:, 0].data, indices[:, 1].data, :, :]  # [nPosRoIs, h, w]

        if ignore_bg:
            y_true_flatten = y_true.view(-1,)  # [nPosRoIs * h * w]
            y_bin_flatten = y_bin.view(-1,)    # [nPosRoIs * h * w]
            y_pred_flatten = y_pred.view(-1,)  # [nPosRoIs * h * w]

            bin_mask = torch.gt(y_bin_flatten, 0)  # greater than 0, [nPosRoIs * h * w]
            y_true = y_true_flatten[bin_mask]  # [nPosRoIs * h' * w']
            y_pred = y_pred_flatten[bin_mask]  # [nPosRoIs * h' * w']
...
def compute_losses(rpn_match, rpn_bbox, rpn_class_logits, rpn_pred_bbox, target_class_ids, mrcnn_class_logits,
                   target_deltas, mrcnn_bbox, target_mask, mrcnn_mask, target_bin_drawing, ignore_bg=False):
    rpn_class_loss = compute_rpn_class_loss(rpn_match, rpn_class_logits)
    rpn_bbox_loss = compute_rpn_bbox_loss(rpn_bbox, rpn_match, rpn_pred_bbox)
    mrcnn_class_loss = compute_mrcnn_class_loss(target_class_ids, mrcnn_class_logits)
    mrcnn_bbox_loss = compute_mrcnn_bbox_loss(target_deltas, target_class_ids, mrcnn_bbox)
    mrcnn_mask_loss = compute_mrcnn_mask_loss(target_mask, target_class_ids, mrcnn_mask, target_bin_drawing, ignore_bg)
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, image_index):
        ...
        image, image_metas, gt_class_ids, gt_boxes, gt_masks, gt_bin_drawings = \
        ...
        images = mold_image(image.astype(np.float32), self.config)  # [0-255] - mean, [H, W, 3]
        ...
        gt_bin_drawings = torch.from_numpy(gt_bin_drawings.astype(int).transpose(2, 0, 1)).float()

        return images, image_metas, rpn_match, rpn_bbox, gt_class_ids, gt_boxes, gt_masks, gt_bin_drawings
...
class MaskRCNN(nn.Module):
    ...
    def set_log_dir(self, model_path=None):
        ...
        if model_path:
            ...
            # 'outputs/snapshot/V1_1/mask_rcnn_sketchyscene_0050.pth'
            regex = r".*/mask\_rcnn\_\w+(\d{4})\.pth"
        ...
        self.checkpoint_path = os.path.join(self.model_dir, "mask_rcnn_{}_*epoch*.pth".format(
    ...
    def load_weights(self, filepath, excludes=None):
    ...
    def predict(self, input, mode='inference'):
    ...
    def collate_custom(self, batch):
        ...
        gt_bin_drawings = [item[7] for item in batch]

        return [images, image_metas, rpn_match, rpn_bbox, gt_class_ids, gt_boxes, gt_masks, gt_bin_drawings]

    def train_model(self, train_dataset, learning_rate, epochs, layers, logger=None):
        """Train the model.
        ...
        opt_params = [{'params': trainables_wo_bn, 'weight_decay': self.config.WEIGHT_DECAY},
                      {'params': trainables_only_bn}]
        if self.config.TRAIN_OPTIMIZER == 'sgd':
            optimizer = optim.SGD(opt_params, lr=learning_rate, momentum=self.config.LEARNING_MOMENTUM)
        ...
        for epoch in range(self.epoch + 1, epochs + 1):
            ...
            for inputs in train_generator:
                ...
                for i in range(self.config.BATCH_SIZE):
                    ...
                    gt_bin_drawings = gt_bin_drawings_batch[i]
                    ...
                    gt_bin_drawings = gt_bin_drawings.unsqueeze(0)
                    ...
                    if self.config.GPU_COUNT:
                        ...
                        gt_bin_drawings = gt_bin_drawings.cuda()
                    ...
                    rois, target_class_ids, target_deltas, target_mask, target_bin_drawings = \
                        detection_target_layer(rpn_rois, gt_class_ids, gt_boxes, gt_masks, gt_bin_drawings, self.config)
                    ...
                    if not rois.size()[0]:
                        ...
                        # Network Heads
                        # Proposal classifier and BBox regressor heads
                        mrcnn_feature_maps = [p2_out[i].unsqueeze(0), p3_out[i].unsqueeze(0), p4_out[i].unsqueeze(0),
                ...
                if step % self.config.STEPS_PER_EPOCH == 0:
                    ...
                    left_sec = left_epochs * duration_time
