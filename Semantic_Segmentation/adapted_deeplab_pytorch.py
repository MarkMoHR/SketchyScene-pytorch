...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=47, up_mode='bilinear'):
        self.inplanes = 64
        ...
        self.fc1_sketch46_c0 = nn.Conv2d(512 * block.expansion, num_classes, kernel_size=3,
                                         stride=1, dilation=6, padding=6)  # deeplab change
        self.fc1_sketch46_c1 = nn.Conv2d(512 * block.expansion, num_classes, kernel_size=3,
                                         stride=1, dilation=12, padding=12)  # deeplab change
        self.fc1_sketch46_c2 = nn.Conv2d(512 * block.expansion, num_classes, kernel_size=3,
                                         stride=1, dilation=18, padding=18)  # deeplab change
        self.fc1_sketch46_c3 = nn.Conv2d(512 * block.expansion, num_classes, kernel_size=3,
                                         stride=1, dilation=24, padding=24)  # deeplab change

        # sketchyscene change
        self.up_mode = up_mode
        if self.up_mode == 'deconv':
            self.deconv = nn.ConvTranspose2d(num_classes, num_classes, kernel_size=16, stride=8, padding=5)
    ...
    def forward(self, x):
        ...
        x0 = self.fc1_sketch46_c0(x)
        x1 = self.fc1_sketch46_c1(x)
        x2 = self.fc1_sketch46_c2(x)
        x3 = self.fc1_sketch46_c3(x)

        x = torch.add(x0, x1)
        x = torch.add(x, x2)
        x = torch.add(x, x3)  # shape = [1, nClasses, H/8, W/8]

        # sketchyscene change
        if self.up_mode == 'deconv':
            x = self.deconv(x)  # shape = [1, nClasses, H, W]
